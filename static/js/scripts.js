    ...
    $('#export2-btn').click(function () {
          ...
          $.ajax({
              ...
              success: function(response) {
                  // Handle file download
                  var blob = new Blob([response], { type: 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet' });
                  var fileName = `Bobovdol_AMS_ALL_${startDate}_${endDate}.xlsx`;
                  var link = document.createElement('a');
                  link.href = window.URL.createObjectURL(blob);
                  link.download = fileName;
                  link.click();
...
//        loadingMessage.show();
//        container.find('div').not('#loading-message-24').remove(); // Clear existing content but keep the loading message
//
//        // Simulate a short delay to mimic data processing/loading
//        setTimeout(() => {

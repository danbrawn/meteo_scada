$(document).ready(function () {
    ...
    function startFetchInterval() {
        ...
        fetchInterval = setInterval(fetchMomentData, 60000);
    }
    ...
    function fetchMomentData() {
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                // Update Last 24 Hours Plot Container with Plot Images
                if (response.plots && response.plots.length > 0) {
                    const plotContainer = $('#last-24-hours-plot');
                    plotContainer.empty(); // Clear existing images
                    response.plots.forEach(plotPath => {
                        const imgElement = `<img src="${plotPath}" alt="Plot Image" class="plot-image" style="margin: 10px; max-width: 800px;">`;

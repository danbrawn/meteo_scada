...
// Function to render a dashboard with key-value pairs
function renderDashboard(data, containerId, columnOrder) {
        ...
        if (key in data) { // Ensure the key exists in the data
            const dashboardItem = `
                <div class="dashboard-item">
                    ...
                    <div class="variable-value">${data[key]}</div>
                    <div class="variable-unit">${key}</div>
                </div>
            `;
    ...
    // Function to fetch and update the moment data
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                if (response.status_data && response.status_data.length > 0) {
                    renderDashboard(response.status_data[0], 'last-min-status-dashboard', response.columns_status);
                }

                // Render Last Minute Values Dashboard
                if (response.min_values_data && response.min_values_data.length > 0) {
                    renderDashboard(response.min_values_data[0], 'last-min-values-dashboard', response.columns_values);
                }

                // Render Last Hour Values Dashboard
                if (response.hour_values_data && response.hour_values_data.length > 0) {
                    renderDashboard(response.hour_values_data[response.hour_values_data.length - 1], 'last-hour-values-dashboard', response.columns_values);
                }
